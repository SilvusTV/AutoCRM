        ...
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                ...
                @if($declarationFrequency === 'monthly')
                    {{ Carbon::createFromDate($declaration->year, $declaration->month, 1)->format('F Y') }}
                @elseif($declarationFrequency === 'quarterly')
                    Q{{ $declaration->month }} {{ $declaration->year }}
            ...
            <div class="flex space-x-2">
                <a href="{{ route('urssaf-declarations.edit', $declaration->id) }}"
                   class="inline-flex items-center px-4 py-2 bg-yellow-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-700 focus:bg-yellow-700 active:bg-yellow-900 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    {{ __('Modifier') }}
                </a>
                <a href="{{ route('urssaf-declarations.index') }}"
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <p class="font-medium">
                                ...
                                @elseif($declarationFrequency === 'quarterly')
                                    @php
                                        $quarterStart = ($declaration->month - 1) * 3 + 1;
                                        $quarterEnd = $declaration->month * 3;
                                        $startMonth = Carbon::createFromDate($declaration->year, $quarterStart, 1)->format('F');
                                        $endMonth = Carbon::createFromDate($declaration->year, $quarterEnd, 1)->format('F');
                                    @endphp
                                    Q{{ $declaration->month }} {{ $declaration->year }} ({{ $startMonth }}
                                    - {{ $endMonth }})
                                @elseif($declarationFrequency === 'annually')
                        ...
                        <div>
                            ...
                            <p class="font-medium">{{ number_format($declaration->declared_revenue, 2, ',', ' ') }}
                                €</p>
                        </div>
                    ...
                    <div class="mt-6 flex justify-end space-x-2">
                        <form action="{{ route('urssaf-declarations.destroy', $declaration->id) }}" method="POST"
                              class="inline"
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                        ...
                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-2">
                            Pour payer cette déclaration, connectez-vous à votre espace personnel sur le site de
                            l'URSSAF :
                            <a href="https://www.autoentrepreneur.urssaf.fr/" target="_blank"
                               class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">
                                www.autoentrepreneur.urssaf.fr
                            </a>
                        </p>
                        <p class="text-sm text-gray-600 dark:text-gray-400">
                            Une fois le paiement effectué, n'oubliez pas de mettre à jour le statut de cette
                            déclaration.
                        </p>
                    </div>
                    ...
                    @if(!$declaration->is_paid)
                        <div class="mt-4">
                            <form action="{{ route('urssaf-declarations.update', $declaration->id) }}" method="POST"
                                  class="space-y-4">
                                @csrf
                                @method('PUT')
                                ...
                                <input type="hidden" name="declared_revenue"
                                       value="{{ $declaration->declared_revenue }}">
                                ...
                                <div>
                                    ...
                                    <x-text-input id="payment_date" class="block mt-1 w-full" type="date"
                                                  name="payment_date" :value="now()->format('Y-m-d')" required/>
