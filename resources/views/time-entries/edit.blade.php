        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('time-entries.update', $timeEntry->id) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <x-text-input id="date" class="block mt-1 w-full" type="date" name="date" :value="old('date', $timeEntry->date->format('Y-m-d'))" required />
                        ...
                        <div>
                            ...
                            <div class="mt-2 space-y-4">
                                <div class="flex items-center">
                                    <input id="entry_type_duration" type="radio" name="entry_type" value="duration" class="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500" 
                                        {{ old('entry_type', (!$timeEntry->start_time && !$timeEntry->end_time) ? 'duration' : '') === 'duration' ? 'checked' : '' }} />
                                    <label for="entry_type_duration" class="ml-2 block text-sm text-gray-900 dark:text-gray-100">
                                        Durée totale
                                    </label>
                                </div>
                                <div class="flex items-center">
                                    <input id="entry_type_time_range" type="radio" name="entry_type" value="time_range" class="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500" 
                                        {{ old('entry_type', ($timeEntry->start_time && $timeEntry->end_time) ? 'time_range' : '') === 'time_range' ? 'checked' : '' }} />
                        ...
                        <div id="duration_fields" class="space-y-4">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <x-text-input id="duration_hours" class="block mt-1 w-full" type="number" name="duration_hours" 
                                        :value="old('duration_hours', floor($timeEntry->duration_minutes / 60))" min="0" />
                                ...
                                <div>
                                    ...
                                    <x-text-input id="duration_minutes" class="block mt-1 w-full" type="number" name="duration_minutes" 
                                        :value="old('duration_minutes', $timeEntry->duration_minutes % 60)" min="0" max="59" />
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <x-text-input id="start_time" class="block mt-1 w-full" type="time" name="start_time" 
                                        :value="old('start_time', $timeEntry->start_time ? $timeEntry->start_time->format('H:i') : '')" />
